...
static u8 USART1_RX_BUF[USART_TX_BUFFER_SIZE];
static u8 USART1_TX_BUF[USART_TX_BUFFER_SIZE];
...
static	void       usart1_dma1_channel5_init(void);	
static  os_err_t   uart1_init   (os_device_t* dev);
static  void       usart1_dma1_channel4_init(void);
...
os_err_t usart_register(u16 task_id)
{
	...
	usart_dev->os_device.init  = uart1_init;
	
	usart_dev->os_device.open  = uart1_open;
	usart_dev->os_device.write = uart1_write;

	  
	return os_device_register(&(usart_dev->os_device), USART1_NAME, OS_DEVICE_FLAG_INACTIVATED);
}
...
//波特率设置
  ...
  os_clock_open("DMA1");		
	usart1_dma1_channel4_init();
	usart1_dma1_channel5_init();
  os_clock_close("DMA1");	
...
static os_size_t  uart1_write(os_device_t* dev, os_off_t pos, const void *buffer, os_size_t size)
{
	  ...
	  if(pos>=size||buffer==NULL||size>USART_TX_BUFFER_SIZE)
		...
		}else
		{
				...
				osmemcpy(USART1_TX_BUF,(char *)buffer, size);
			  USART1TX_DMA1_Channelx->CNDTR=size;//DMA channel x number of data register
        USART1TX_DMA1_Channelx->CMAR=(unsigned long)(USART1_TX_BUF);
				USART1TX_DMA1_Channelx->CCR|=1<<0;          //开启DMA传输
		}
 ...
 *   @return none
 */
static void usart1_dma1_channel4_init()
{
 ...
 *   @return none 
 */
static void usart1_dma1_channel5_init()
{
   
	
	USART1RX_DMA1_Channelx->CCR=0;//reset
	USART1RX_DMA1_Channelx->CCR|=0<<10;//Memory size 0:8bits,16-bits	 2 32bits
	USART1RX_DMA1_Channelx->CCR|=0<<8;//Peripheral size 0:8bits,1:16-bits	 2 32bits
	USART1RX_DMA1_Channelx->CCR|=1<<7;//Memory increment mode enabled
	USART1RX_DMA1_Channelx->CCR|=2<<12;//
	USART1RX_DMA1_Channelx->CCR|=1<<5;//Memory Circular  mode enabled
	
	USART1RX_DMA1_Channelx->CCR&=~(1<<6);	//Peripheral increment mode disabled
	USART1RX_DMA1_Channelx->CCR&=~(1<<4);//Read from peripheral
	//USART1RX_DMA1_Channelx->CCR|=1<<1;//Transfer complete interrupt enable
	USART1RX_DMA1_Channelx->CNDTR=USART_TX_BUFFER_SIZE;//DMA channel x number of data register
  USART1RX_DMA1_Channelx->CMAR=(unsigned long)(USART1_RX_BUF);
	USART1RX_DMA1_Channelx->CPAR=(unsigned long)(&(USART1->DR));
	
	USART1RX_DMA1_Channelx->CCR|=1<<0;          //开启DMA传输
...
void DMA1_Channel4_IRQHandler()
{
		...
		USART1TX_DMA1_Channelx->CCR&=~(1<<0);         
   if(DMA1->ISR&(1<<13))
   {
 ...
 volatile  u32 uart1_status;
  ...
  usart_data usart_message_data;
	...
	{	 
		...
		USART1RX_DMA1_Channelx->CCR&=(~0x01); //disable dma templaroly
		
		data_length=USART_TX_BUFFER_SIZE-USART1RX_DMA1_Channelx->CNDTR; //get the length of data
		
	
		
		usart_message_data.length=data_length;
		osmemcpy(usart_message_data.data, USART1_RX_BUF , data_length);
	...
	//	send_message(usart_dev->register_taskid,DEVICE_EVENT,USART_MSG , NULL,(void*)(usart_message_data), sizeof(usart_message_data));
		...
		USART1RX_DMA1_Channelx->CNDTR=USART_RX_BUFFER_SIZE;
	  USART1RX_DMA1_Channelx->CPAR=(unsigned long)(&USART1->DR);
	  USART1RX_DMA1_Channelx->CMAR=(unsigned long)(USART1_RX_BUF);	
		USART1RX_DMA1_Channelx->CCR|=1<<0; 	
	}  	
