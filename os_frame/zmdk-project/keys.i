...
#line 27 "..\\drivers\\stm32f051x\\misc\\keys\\keys.c"
#line 28 "..\\drivers\\stm32f051x\\misc\\keys\\keys.c"
...
os_err_t keys_register(u16 task_id)
{
    ...
    key_dev->os_device.open=0;
    
    key_dev->os_device.write=0;
    
    
    return os_device_register(&(key_dev->os_device), "keys", OS_DEVICE_FLAG_INACTIVATED);
...
void EXTI9_5_IRQHandler(void)
{
    ...
    key_status.key=key1;
    if(key_trigger_status.key1_trigger==1)
    {
        key_trigger_status.key1_trigger=2;
        
        key_status.status=key_down;
    }else
    {
        key_trigger_status.key1_trigger=1;
        key_status.status=key_released;
    }
    send_message((key_dev->register_taskid),0x04 , 0X03 , 0X02 ,(void*)(&key_status), sizeof(key_data));
    Ex_NVIC_Config(0,8,key_trigger_status.key1_trigger);
    count++;
    ((EXTI_TypeDef *) (((uint32_t)0x40000000) + 0x00010400))->PR=1<<8;   
...
{	
    key_data key_status={0};
    key_status.key=key2;
    if(key_trigger_status.key2_trigger==1)
    {
        key_trigger_status.key2_trigger=2;
        key_status.status=key_down;
    }else
    {
        key_trigger_status.key2_trigger=1;
        key_status.status=key_released;
        
    }
    send_message((key_dev->register_taskid),0x04 , 0X03 , 0X02 ,(void*)(&key_status), sizeof(key_data));
    Ex_NVIC_Config(1,15,key_trigger_status.key2_trigger);
    
    
    ((EXTI_TypeDef *) (((uint32_t)0x40000000) + 0x00010400))->PR=1<<15;     
}
