...
#line 22 "..\\drivers\\stm32f051x\\char\\eeprom\\eeprom.c"
#line 23 "..\\drivers\\stm32f051x\\char\\eeprom\\eeprom.c"

 
 
 
 
static eeprom_device *eeprom_simulation=0;
 
u16 DataVar = 0;

 
u16 VirtAddVarTab[((uint8_t)0x03)];
...
os_err_t eeprom_register(u16 task_id)
{
    ...
    if(eeprom_simulation == 0)
    {
        

        printf("eeprom allocated failed");

        return ERROR;
        
    }
    
    eeprom_simulation->pclk2=48;
    
    eeprom_simulation->bound=115200;
    ...
    return os_device_register(&(eeprom_simulation->os_device), "EEPROM", OS_DEVICE_FLAG_INACTIVATED);
}
...
os_size_t eeprom_read   (os_device_t* dev, os_off_t pos, void *buffer, os_size_t size)
{
    ...
    for(data_num =0 ; data_num < size_16bit ; data_num++)
    {
        ...
        if(read_status == ERROR || read_status == ((uint16_t)0x00AB) )
        {
					

					
					  printf("EEPROM Read failed[%s]:%d","..\\drivers\\stm32f051x\\char\\eeprom\\eeprom.c", 284 );
...
static u16 EE_Init(void)
{
    ...
    PageStatus0 = (*(volatile u16*)((uint32_t)(((uint32_t)0x0800E000) + 0x0000)));
     
    PageStatus1 = (*(volatile u16*)((uint32_t)(((uint32_t)0x0800E000) + 0x0400)));
    
     
    switch (PageStatus0)
    {
    case ((uint16_t)0xFFFF):
        if (PageStatus1 == ((uint16_t)0x0000))  
        {
             
            FlashStatus = FLASH_ErasePage(((uint32_t)(((uint32_t)0x0800E000) + 0x0000)));
        ...
        else if (PageStatus1 == ((uint16_t)0xEEEE))  
        {
            ...
            FlashStatus = FLASH_ProgramHalfWord(((uint32_t)(((uint32_t)0x0800E000) + 0x0400)), ((uint16_t)0x0000));
    ...
    case ((uint16_t)0xEEEE):
        if (PageStatus1 == ((uint16_t)0x0000))  
        {
             
            for (VarIdx = 0; VarIdx < ((uint8_t)0x03); VarIdx++)
            {
                if (( *(volatile u16*)(((uint32_t)(((uint32_t)0x0800E000) + 0x0000)) + 6)) == VirtAddVarTab[VarIdx])
                {
                    x = VarIdx;
                }
                if (VarIdx != x)
            ...
            FlashStatus = FLASH_ProgramHalfWord(((uint32_t)(((uint32_t)0x0800E000) + 0x0000)), ((uint16_t)0x0000));
            ...
            FlashStatus = FLASH_ErasePage(((uint32_t)(((uint32_t)0x0800E000) + 0x0400)));
        ...
        else if (PageStatus1 == ((uint16_t)0xFFFF))  
        {
    ...
    case ((uint16_t)0x0000):
        ...
        else  
        {
            ...
            for (VarIdx = 0; VarIdx < ((uint8_t)0x03); VarIdx++)
            {
                if ((*(volatile u16*)(((uint32_t)(((uint32_t)0x0800E000) + 0x0400)) + 6)) == VirtAddVarTab[VarIdx])
                {
                    x = VarIdx;
                }
                if (VarIdx != x)
...
static u16 EE_ReadVariable(u16 VirtAddress, u16* Data)
{
    u16 ValidPage = ((uint16_t)0x0000);
    u16 AddressValue = 0x5555, ReadStatus = ERROR;
    u32 Address = 0x08010000, PageStartAddress = 0x08010000;
    
     
    ValidPage = EE_FindValidPage(((uint8_t)0x00));
    
     
    if (ValidPage == ((uint16_t)0x00AB))
    {
        return  ((uint16_t)0x00AB);
    }
    
     
    PageStartAddress = (u32)(((uint32_t)0x0800E000) + (u32)(ValidPage * ((uint32_t)0x0400)));
    
     
    Address = (u32)((((uint32_t)0x0800E000) - 2) + (u32)((1 + ValidPage) * ((uint32_t)0x0400)));
    
     
    while (Address > (PageStartAddress + 2))
    {
         
        AddressValue = (*(volatile u16*)Address);
        
         
        if (AddressValue == VirtAddress)
        {
             
            *Data = (*(volatile u16*)(Address - 2));
...
static u16 EE_FindValidPage(u8 Operation)
{
    ...
    case ((uint8_t)0x01):    
        if (PageStatus1 == ((uint16_t)0x0000))
        {
            ...
            if (PageStatus0 == ((uint16_t)0xEEEE))
            {
                return ((uint16_t)0x0000);          
            }
            else
            {
                return ((uint16_t)0x0001);          
            }
        }
        else if (PageStatus0 == ((uint16_t)0x0000))
        {
             
            if (PageStatus1 == ((uint16_t)0xEEEE))
            {
                return ((uint16_t)0x0001);          
            }
            else
    ...
    case ((uint8_t)0x00):   
        if (PageStatus0 == ((uint16_t)0x0000))
        {
            return ((uint16_t)0x0000);            
        }
        else if (PageStatus1 == ((uint16_t)0x0000))
        {
            return ((uint16_t)0x0001);            
        }
        else
...
static u16 EE_VerifyPageFullWriteVariable(u16 VirtAddress, u16 Data)
{
    ...
    ValidPage = EE_FindValidPage(((uint8_t)0x01));
    ...
    Address = (u32)(((uint32_t)0x0800E000) + (u32)(ValidPage * ((uint32_t)0x0400)));
    
     
    PageEndAddress = (u32)((((uint32_t)0x0800E000) - 2) + (u32)((1 + ValidPage) * ((uint32_t)0x0400)));
    
     
    while (Address < PageEndAddress)
    {
         
        if ((*(volatile u32*)Address) == 0xFFFFFFFF)
        {
...
static u16 EE_PageTransfer(u16 VirtAddress, u16 Data)
{
    ...
    u16 ValidPage = ((uint16_t)0x0000), VarIdx = 0;
    u16 EepromStatus = 0, ReadStatus = 0;
    
     
    ValidPage = EE_FindValidPage(((uint8_t)0x00));
    
    if (ValidPage == ((uint16_t)0x0001))        
    {
         
        NewPageAddress = ((uint32_t)(((uint32_t)0x0800E000) + 0x0000));
        
         
        OldPageAddress = ((uint32_t)(((uint32_t)0x0800E000) + 0x0400));
    }
    else if (ValidPage == ((uint16_t)0x0000))   
    {
         
        NewPageAddress = ((uint32_t)(((uint32_t)0x0800E000) + 0x0400));
        
         
        OldPageAddress = ((uint32_t)(((uint32_t)0x0800E000) + 0x0000));
    }
    else
    {
        return ((uint16_t)0x00AB);        
    ...
    FlashStatus = FLASH_ProgramHalfWord(NewPageAddress, ((uint16_t)0xEEEE));
    ...
    FlashStatus = FLASH_ProgramHalfWord(NewPageAddress, ((uint16_t)0x0000));
