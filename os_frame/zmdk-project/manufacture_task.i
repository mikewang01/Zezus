...
#line 31 "..\\task\\usertask\\manufacture_task.c"
#line 32 "..\\task\\usertask\\manufacture_task.c"
#line 1 "..\\task\\usertask\\manufacture_task.h"



#line 5 "..\\task\\usertask\\manufacture_task.h"
#line 6 "..\\task\\usertask\\manufacture_task.h"
...
#line 33 "..\\task\\usertask\\manufacture_task.c"
#line 34 "..\\task\\usertask\\manufacture_task.c"
...
void task_manufacture()
{
    ...
    do{ task_state=OS_get_taskstate(); task_state=task_state +My_Id; }while(0);
    
    
     
    if(((task_state->State)&0x04)==0x04)
    {
    ...
    delete_message(My_Id,0x04);
    
     
    do{ (OS_get_taskstate())[My_Id]. task_tick++; }while(0);
}
...
static void process_msg(u16 taskid , u16 msg_num , smessage* msg)
{
    do{
        ...
        case 0X02: 
            ...
            }else if((msg->event&0x05)==0x05)
            {
                
                
            }
            else if((msg->event&0x06)==0x06)
            {
                
                
            }					 
            else if((msg->event&0X08)==0X08)
            {
...
static os_err_t process_order(data_packge package)
{
    os_device_t *dev_serial = os_device_get("USART1");
		...
		if((dev_serial!=0)) 0; else exam_assert( "..\\task\\usertask\\manufacture_task.c", 225 );
    ...
    if(package.order == get_mac)
    {
			   ...
			   os_device_t *dev = os_device_get("me2_ch2o");
			    ...
			    u8 temp[10]={0 , 11,22,33,44,55,66,77,88}; 
			   	temp[0]= sizeof(correction_para);
        ...
        package.payload.lenth = sizeof(mac_adress);
        
         
        os_memmove(package.payload.pay_load , mac_adress , package.payload.lenth);
        
			  
         
        return os_device_write(dev_serial , 0 , (u8*)(&package) , sizeof(data_packge)); 
        
    }else if(package.order == get_vol)  
    {
        ...
        *((u16*)package.payload.pay_load) = sensor_get_voltage();
    ...
    }else if(package.order == get_dsy) 
		{
			   ...
			   os_device_t *dev = os_device_get("me2_ch2o");
			    ...
			    if((dev!=0)) 0; else exam_assert( "..\\task\\usertask\\manufacture_task.c", 275 );
         ...
         os_memmove(package.payload.pay_load , (void*)&density , package.payload.lenth);
			   ...
			   os_device_control(dev, update_correction_sensor , 0);
				 
				   
         return os_device_write(dev_serial , 0 , (u8*)(&package) , sizeof(data_packge));
				 
		}else if(package.order == set_para)  
		{
			  ...
			  if((dev!=0)) 0; else exam_assert( "..\\task\\usertask\\manufacture_task.c", 300 );
    ...
    }else if(package.order == get_para)  
    {
			  
			  
    }else if(package.order == get_hard_version)  
		{
				
		}
		else if(package.order == get_soft_version)  
		{
      u32 version;
			 
			version = ((0<<24) | 1<<16 | 0<<8 | 0);
