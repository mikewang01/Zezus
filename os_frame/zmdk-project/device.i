...
#line 15 "..\\kernel\\devmngr\\device.c"
...
#line 16 "..\\kernel\\devmngr\\device.c"
...
static struct os_object *object_dev_list;
...
{   
    ...
    if (os_object_find(object_dev_list,name) != 0)
        return ERROR;   
    dev->flag = flags;
...
os_err_t os_device_init_all(void)
{
    ...
    {	
        ...
        device=(os_device_t *)( (char *)__mptr - ((size_t) &((os_device_t *)0)->parent));
        if(device->init!=0)
        {
...
{  
    ...
    }	else
    {
        ...
        if(obj!=0)
        {
            ...
            return ((os_device_t *)( (char *)__mptr - ((size_t) &((os_device_t *)0)->parent)));
        }
    }
    return 0;
} 	
...
os_err_t _list_device()
{
    ...
    printf("device    type      \r\n");
    printf("-------- ---------- \r\n");
    for (; __mptr!=0; __mptr = __mptr->next)
    {
        struct rt_device *device=(os_device_t *)( (char *)__mptr - ((size_t) &((os_device_t *)0)->parent));
        printf("%-8.*s %-8s \r\n",
               10,
               device->parent.name,
